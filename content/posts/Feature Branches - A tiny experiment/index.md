...
hideComments = false
+++
At my (at the time of writing) current workplace, we have what could be best described as the classic "trunk based" development pattern.
...
1. Making a release branch was a tricky process, especially if changes were in main that were not ready to be released (seems strange on the face of it but I'll explain)
...
2. A new branch would be made from this one like `JIRABOARD-1234`
...
5. You would continue building your feature on top of your existing branch (`JIRABOARD-1234`) for a different ticket, for example `JIRABOARD-1235`
6. You would open a PR to `feat/my-epic-feature` when you were done working on `JIRABOARD-1235`
...
### It didn't work :/
To put it bluntly, it didn't work. In it's failure though it helped quantify other issues we have in our workflows
...
3. Reviews! I think the biggest hurdle to velocity will always be reviews (and I am glad for that). Careful considered reviews are probably the most valuable thing a team can have
    1. We had a 2 review requirement for anything going to main, this now added an additional review cycle (the rubber stamp) to get something to main
    2. Anytime the feature branch went out of sync with main and needed to be reconciled (either by merging or rebasing), it required another set of reviews to fix (since the feature branch was a protected branch, needed to enable the 2 review requirement)
